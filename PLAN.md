...
- По пътя са фиксирани три универсални bug-а (не bagabuch-специфични):
  ...
  - boilaDB `c7d5fbb`: числови низове с водещи нули (ЕИК/фактурни номера) вече не се
    инференцират като цяло число — запазват се като текст

### Фаза 3 — Frontend (Next.js) ✓
- Глобално от самото начало: светла/тъмна тема (CSS променливи, запазва се, без проблясък) и български дефолт + английски (речници, запазват се)
- Провайдъри: тема, език, автентикация (useSyncExternalStore, хидратационно-безопасни)
- Страници: вход, табло, фирми, сметкоплан, контрагенти, стоки, фактури (с редове и автоматични суми), дневник (разгъващи се дебит/кредит редове)
- Генеричен CRUD компонент; частичен PATCH; автентикация с токен
- Мултитенант: активна фирма по потребител (в сеттингс), превключвател и показване в хедъра + бутон «Активирай» в страницата с фирми
- Приемане: `npm run build` + `npm run lint` чисти, страниците се рендерират
